...
class DD [T: ClassTag] {
  ...
  def test(inputRDD: RDD[T], testFunc: userTest[T], test_num: Int, lm: LogManager, fh: FileHandler): Boolean = {
    testFunc.usrTest(inputRDD, test_num, lm, fh)
  }
  ...
  def test(inputRDD: Array[T], testFunc: userTest[T], test_num: Int, lm: LogManager, fh: FileHandler): Boolean = {
    testFunc.usrTest(inputRDD, test_num, lm, fh)
  }
  ...
  private def dd_helper(inputRDD: RDD[T],
                        ...
                        lm: LogManager,
                        fh: FileHandler,
                        skipList: List[Boolean]): List[Boolean] = {
          ...
          while (!failing_stack.isEmpty) {
            breakable {
              ...
              if (sizeRdd < dd_movetolocal_threshold && runningOnCluster) {
                ...
                returnedList = localRDD(rdd.collect(), numberOfPartitions, testFunc, splitFunc, lm, fh, i, returnedList)
              ...
              val assertResult = test(rdd, testFunc, i, lm, fh)
              runTime = runTime + 1
              ...
              for (i <- 0 until partitions) {
                ...
                val result = test(rddList(i), testFunc, i, lm, fh)
                runTime = runTime + 1
                if (i == 0) {
                  first_rdd_runTime = first_rdd_runTime + 1
                }
              ...
              if (!rdd_failed) {
                for (j <- 0 until partitions) {
                  ...
                  val result = test(rddBar, testFunc, i, lm, fh)
                  runTime = runTime + 1
                  if (result) {
                    rddBar_failed = true
  ...
  def ddgen(inputRDD: RDD[T], testFunc: userTest[T], splitFunc: userSplit_v2[T], lm: LogManager, fh: FileHandler, skipList: List[Boolean]): List[Boolean] = {
    dd_helper(inputRDD, 2, testFunc, splitFunc, lm, fh, skipList)
  }
  ...
  def localRDD(inputRDD: Array[T],
    ...
    while (!failing_stack.isEmpty) {
      breakable {
        ...
        println(s""">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> $sizeRdd <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<""")
        val assertResult = test(rdd, testFunc, test_num, lm, fh)
        runTime = runTime + 1
        if (!assertResult) {
          val endTime: Long = System.nanoTime
        ...
        for (i <- 0 until partitions) {
          //          println("Testing subRDD id:" + rddList(i).id)
          val result = test(rddList(i), testFunc, test_num, lm, fh)
          runTime = runTime + 1
          if (result) {
            rdd_failed = true
            next_partitions = 2
        ...
        if (!rdd_failed) {
          for (j <- 0 until partitions) {
            ...
            val result = test(rddBar, testFunc, test_num, lm, fh)
            runTime = runTime + 1
            if (result) {
              rddBar_failed = true
              //              next_rdd = next_rdd.intersection(rddBar)
